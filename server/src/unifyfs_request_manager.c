...
// QUESTION: Pass app & client id's, or just reqmgr and init id's here?
// QUESTION: Locks here and/or in process loop for main thread?
void queue_cleanup(reqmgr_thrd_t* thrd_ctrl)
{
    //insert app_id and client_id in cleanup_arraylist[]

    cleanup_arraylist = NULL;

    RM_REQ_LOCK(thrd_ctrl);

    int* app_id = (void*)&thrd_ctrl->app_id;
    int* client_id = (void*)&thrd_ctrl->client_id;

    arraylist_insert(cleanup_arraylist, 0, app_id);
    arraylist_insert(cleanup_arraylist, 1, client_id);
